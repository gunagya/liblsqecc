...
std::vector<Gate> approximate_RZ_gate_cached(const RZ rz_gate)
{
    std::vector<Gate> res;
    if (std::holds_alternative<Fraction>(rz_gate.angle))
        throw std::runtime_error{lstk::cat("Not implemented! No cached decompositions for decimal angles.")};
    Fraction pi_fraction = std::get<Fraction>(rz_gate.angle);
    ...
    throw std::runtime_error{lstk::cat("Can only approximate pi/2^n phase gates in cached mode, got rz(", print_pi_fraction(pi_fraction),")\n"
